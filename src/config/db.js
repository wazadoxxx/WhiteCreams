...
const hasConfiguredSupabase = Boolean(
  databaseUrl || (pgHost && pgUser && pgDatabase && pgPassword)
);
...
if (fs.existsSync(initSqlPath)) {
  ...
  const isPostgresOnlyInitSql = /(GENERATED BY DEFAULT AS IDENTITY|TIMESTAMPTZ|JSONB|::jsonb|setval\s*\()/i.test(initSql);

  if (!isPostgresOnlyInitSql) {
    try {
      db.exec(initSql);
    } catch (error) {
...
db.pgPool = pgPool;
db.hasConfiguredSupabase = hasConfiguredSupabase;
db.hasConfiguredPostgres = hasConfiguredSupabase;
db.supabaseUrl = supabaseUrl || null;
