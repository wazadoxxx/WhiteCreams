...
app.get('/api/player/:id', async (req, res) => {
  ...
  if (!Number.isInteger(playerId) || playerId <= 0) {
    return res.status(400).json({
      ok: false,
      error: 'Identifiant joueur invalide.'
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: 'Erreur serveur pendant le chargement du joueur.',
...
app.get('/api/activities/:playerId', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: 'Erreur serveur pendant le chargement des activites.',
      details: error.message
    });
  }
});
...
app.post('/api/activity', async (req, res) => {
  ...
  if (!Number.isFinite(parsedQuantity) || !Number.isFinite(parsedGain)) {
    return res.status(400).json({
      ok: false,
      error: 'Quantite et gain invalides.'
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: 'Erreur serveur pendant la creation de l activite.',
      details: error.message
    });
  }
});
    ...
    if (pgPool) {
      try {
        ...
        response.postgres = {
          ...
          currentUser: pgRow.user_name || null,
          currentTime: pgRow.server_time || null,
          error: null
        };
      } catch (pgError) {
        response.postgres = {
